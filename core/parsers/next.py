...
# --- Handle Markdown files ---
    ...
    if ext in [".ppt", ".pptx"]:
        try:
            await safe_emit(
                f"{user_id}/progress",
                {"message": f"{title} is a PowerPoint, extracting slides..."},
            )
            ...
            for slide_num, slide in enumerate(presentation.slides, start=1):
                ...
                content.append(Page(number=slide_num, text=slide_text))
                full_text += slide_text + "\n"

            # If no text found, attempt export + OCR fallback
            if not full_text.strip():
                print("No text found in slides. Attempting OCR fallback...")
                ocr_pages = await export_and_ocr_ppt_with_fallback(file_path)

                content = []
                full_text = ""

                for i, page_text in enumerate(ocr_pages, start=1):
                    content.append(Page(number=i, text=page_text))
                    full_text += page_text + "\n"

        except Exception as e:
            print(f"Error processing PowerPoint {safe_file_name}: {str(e)}")
            traceback.print_exc()
            return None
    ...
    if ext in {".ppt", ".pptx"}:
        ...
        try:
            for slide_number, slide in enumerate(prs.slides, start=1):
                ...
                for shape in slide.shapes:
                    try:
                        # Check for SmartArt objects
                        if hasattr(shape, "smart_art") and shape.smart_art:
                            smartart_text = []
                            try:
                                for node in shape.smart_art.nodes:
                                    if hasattr(node, "text_frame"):
                                        for paragraph in node.text_frame.paragraphs:
                                            if paragraph.text.strip():
                                                smartart_text.append(paragraph.text.strip())
                                if smartart_text:
                                    slide_text.append(f"\n[SmartArt Diagram]\n" + "\n".join(smartart_text) + "\n[/SmartArt Diagram]")
                            except Exception as e:
                                print(f"Error extracting SmartArt from slide {slide_number}: {e}")
        ...
        # --- Full Slide OCR using LibreOffice (captures grouped shapes, flowcharts, etc.) ---
        await safe_emit(
            f"{user_id}/progress", {"message": f"Performing full slide OCR for {title}..."}
        )

        print(f"[Full Slide OCR] Starting LibreOffice-based OCR for {safe_file_name}")

        full_slide_ocr_results = await export_and_ocr_ppt_with_fallback(
            file_path, user_id, thread_id
        )

        if full_slide_ocr_results:
            print(f"[Full Slide OCR] Successfully OCR'd {len(full_slide_ocr_results)} slides")

            # Merge full slide OCR results with existing content
            for i, ocr_text in enumerate(full_slide_ocr_results, start=1):
                if i <= len(pages):
                    # Add full slide OCR to existing page text
                    if ocr_text and ocr_text.strip():
                        pages[i-1].text += f"\n\n[Full Slide OCR]\n{ocr_text}\n[/Full Slide OCR]"
                        combined_texts[i-1] = pages[i-1].text
        else:
            print("[Full Slide OCR] LibreOffice not available or failed, using existing extraction only")
