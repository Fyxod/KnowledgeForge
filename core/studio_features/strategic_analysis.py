...
from core.constants import GPU_STRATEGIC_ANALYSIS_LLM, MODEL_INPUT_BUDGET
...
async def generate_strategic_analysis(
    ...
    """
    Generate a strategic analysis based on the provided document.

    Args:
        document: The document (or list of documents) to analyze.

    Returns:
        StrategicAnalysisLLMOutput: The generated strategic analysis.
    """
    document_text = fetch_document_content(document)

    prompt = build_strategic_analysis_prompt(document_text)

    response: StrategicAnalysisLLMOutput = await invoke_llm(
        gpu_model=GPU_STRATEGIC_ANALYSIS_LLM.model,
        response_schema=StrategicAnalysisLLMOutput,
        contents=prompt,
        port=GPU_STRATEGIC_ANALYSIS_LLM.port,
    )
...
def fetch_document_content(document: Document | list[Document]) -> str:
    ...
    if isinstance(document, Document):
        if hasattr(document, "full_text") and word_count(document.full_text) < 8000:
            print("Using full text for strategic analysis")
            text = document.full_text
        elif hasattr(document, "summary") and document.summary:
            print("Using summary for strategic analysis")
            text = document.summary
        else:
            print("Using truncated text for strategic analysis")
...
def build_strategic_analysis_prompt(document_text: str) -> str:

    prompt = strategic_analysis_prompt(document=document_text)
    return prompt
